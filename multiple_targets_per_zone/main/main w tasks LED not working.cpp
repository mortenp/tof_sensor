...
void data_processor_task(void *pvParameters)
{
    ...
    while (1)
    {
        ...
        if (xQueueReceive(mpu_data_queue, &received_data, portMAX_DELAY) == pdPASS)
        {
            ...
            // --- You can also log the raw data if needed for debugging ---
            // ESP_LOGI("PROCESSOR_TASK", "Raw Accel   -> X: %.2f g, Y: %.2f g, Z: %.2f g", received_data.accel_x_g, received_data.accel_y_g, received_data.accel_z_g);
            // ESP_LOGI("PROCESSOR_TASK", "Raw Gyro    -> X: %.2f °/s, Y: %.2f °/s, Z: %.2f °/s", received_data.gyro_x_dps, received_data.gyro_y_dps, received_data.gyro_z_dps);
            // ESP_LOGI("PROCESSOR_TASK", "Temperature -> %.2f °C", received_data.temp_c);
            // ESP_LOGI("PROCESSOR_TASK", "-------------------------------------------------");
        }
 vTaskDelay(pdMS_TO_TICKS(300));
...
void vl53l5cx_reader_task(void *pvParameters) {
    ...
    const uint32_t SENSOR_READ_INTERVAL = 10; // 1ms
    int loop = 0;
...
// --- Your distance sensing logic ---
        // if (vl53l5cx_check_data_ready(...) == ready) {
...
if ((i % 4) == 0) {
     ...
     if( gridAlerts[i] > 3  ){
        ...
        // 'i' is in the gridTop array.
        // This is equivalent to checking if i is one of {0, 4, 8, 12}.
  //      ESP_LOGI(TAG, "Value %d is in gridTop.", i);
    }
...
if (gridTop[i]) {
//for(int x = 0; x < 4; x++){
     if( gridAlerts[i] > 3  ){ //i == gridTop[x] && 
 //        printf("gridTop Alert %d %d \n", i, gridAlerts[i]);
// ESP_LOGI(TAG, "gridTop %d %d", i, gridAlerts[i]);
//update_beeper_alerts(4);
...
if (gridBottom[i]) {
//for(int x = 0; x < 4; x++){
     if( gridAlerts[i] > 3 ){ //i == gridBottom[i] &&
 //        printf("gridBottom Alert %d %d \n", i, gridAlerts[i]);
// ESP_LOGI(TAG, "gridBottom %d %d", i, gridAlerts[i]);
 //        gpio_set_level(RED_PIN, 0); 
// set_top_bottom_led('b');
bottomAlert = true;
...
/*
for(int x = 0; x < 2; x++){
     if(gridLeft[x] == i && gridAlerts[i] > 3  ){
         printf("gridLeft Alert %d %d \n", i, gridAlerts[i]);
        // gpio_set_level(RED_PIN, 1); 
...
for(int x = 0; x < 2; x++){
     if(gridRight[x] == i && gridAlerts[i] > 3  ){
         printf("gridRight Alert %d %d \n", i, gridAlerts[i]);
        // gpio_set_level(RED_PIN, 1); 
//         gpio_set_level(GREEN_PIN, 1); 
 //         vTaskDelay(pdMS_TO_TICKS(50));   
 //         gpio_set_level(GREEN_PIN, 0); 
...
/*
for(int x = 0; x < 8; x++){
     if( i == gridCentre[x] ){ //&& gridAlerts[i] > 3
 //        printf("gridCentre Alert %d %d \n", i, gridAlerts[i]);
 //        gpio_set_level(RED_PIN, 0); 
  //       gpio_set_level(BLUE_PIN, 1);  
 //        vTaskDelay(pdMS_TO_TICKS(50)); 
 //        gpio_set_level(BLUE_PIN, 0); 
       //  break;
 //       ESP_LOGI(TAG, "gridCentre %d %d", i, gridAlerts[i]);
        primary_distance_total += primary_distance;
  //      printf("gridCentre dist %d tot %d dist %d \n", i, primary_distance_total, primary_distance);
     }
}
*/
...
update_beeper_alerts(alert_level_average);
 ...
 // --- NEW: Create and populate a single, simple LED state object ---
    ...
    // Part 1: Populate the WS2812 part of the state
 //   const alert_config_t *config = &alert_configs[alert_level_average];
...
// --- THIS IS THE NEW DEBUG LOG ---
    ESP_LOGI(TAG, "LED state change: r=%u, g=%u, b=%u, bright=%.2f, top=%s, bot=%s",
             desired_led_state.r,
             desired_led_state.g,
             desired_led_state.b,
             desired_led_state.brightness,
             (desired_led_state.top_led_on ? "ON" : "OFF"), // Use ternary operator for bools
             (desired_led_state.bottom_led_on ? "ON" : "OFF"));
*/  
...
gpio_set_level(RED_PIN, 0); 
gpio_set_level(BLUE_PIN, 0); 
//rgb_led_constant(LedColor::BLACK, 0) ;
...
//esp_task_wdt_feed();
 ...
 //       watchdog_feed_hwdt(); // or watchdog_feed_software()
...
extern "C" void app_main(void)
{
 /*   
    ...
    xTaskCreate(
        ...
        NULL,                 // Parameters
        tskIDLE_PRIORITY,     // Priority (low, as it's monitoring)
...
/*
    // Create the queue to hold 5 items of mpu_data_t.
    mpu_data_queue = xQueueCreate(5, sizeof(mpu_data_t));
    if (mpu_data_queue == NULL) {
        ESP_LOGE("MAIN", "Failed to create MPU data queue!");
        return; // Abort
    }
    ESP_LOGI("MAIN", "MPU data queue created successfully.");
*/
    /*
    // Create the MPU9250 reader task
    xTaskCreate(
        mpu9250_reader_task,        // Task function
        "MPU9250 Reader Task",      // Name for debugging
        4096,                       // Stack size
        (void *)mpu9250_dev_handle, // Pass the device handle as a parameter
        5,                          // Priority
    ...
    xTaskCreate(
        data_processor_task,        // Task function
        "Data Processor Task",      // Name
        4096,                       // Stack size
        NULL,                       // Parameters
        6,                          // Priority
        NULL                        // Task handle
 ...
 // Use the new queue name and create a queue of length 1.
led_state_queue = xQueueCreate(1, sizeof(led_state_t));
if (led_state_queue == NULL) {
    ESP_LOGE("MAIN", "Failed to create LED state queue!");
    return;
}
...
*/
 ...
 //   rgb_led_blink(LedColor::RED, 5, 500, 255);
//rgb_led_blink(LedColor::WHITE, 1, 50, 255);
//rgb_led_blink(LedColor::BLUE, 5, 200, 255);
/*
   vTaskDelay(pdMS_TO_TICKS(50)); 
rgb_led_set_color_with_brightness( 255, 0, 0, 1.000000);
    vTaskDelay(pdMS_TO_TICKS(300)); 
rgb_led_set_color_with_brightness( 0, 255, 0, 1.000000);
    vTaskDelay(pdMS_TO_TICKS(300)); 
rgb_led_set_color_with_brightness( 0, 0, 255, 1.000000);
    vTaskDelay(pdMS_TO_TICKS(300)); 
rgb_led_set_color_with_brightness( 0, 0, 0, 0);
*/


/*
// --- Create the new LED control task alongside your other tasks ---
xTaskCreate(
    led_control_task,
    "LED Control Task",
    4096,
    NULL,
    4, // Priority can be lower than sensors
    NULL
);

*/

    // Your main function can now idle or be deleted if it has nothing else to do.
...
// --- 6. Perform Startup Indications ---
    ...
    for (int i = 0; i < 3; i++) {
     ...
     //     watchdog_feed_software(); // Assuming a software watchdog
 //   xTaskCreate(watchdog_feed_task, "wdt_feed", 2048, NULL, 1, NULL);
